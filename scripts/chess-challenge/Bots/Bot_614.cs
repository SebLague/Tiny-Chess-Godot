...
public class Bot_614 : IChessBot
{
    ...
    public Move Think(Board boardOrig, Timer timerOrig)
    {
        ...
        if (board.PlyCount < 6)
        {
            Move bookMove = TinyOpeningBook.TryGetMove(board, randomlyDontUseBookProb: 0.2);
            if (!bookMove.IsNull)
            {
                return bookMove;
            }
        ...
        while (
            ++searchingDepth <= 200
                && timer.MillisecondsElapsedThisTurn < maxSearchTimeLow
        );
    ...
    public int Negamax(int alpha, int beta, int depth)
    {
        ...
        foreach (Move move in moves)
        {
            ...
            if (inQSearch && eval + (0b1_0100110100_1011001110_0110111010_0110000110_0010110100_0000000000
                                                        >> (int)move.CapturePieceType * 10 & 0b1_11111_11111) <= alpha)
                break;
